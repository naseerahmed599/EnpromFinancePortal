...
def _cache_cost_centers_internal(cost_centers: List[str], months_back: int) -> List[str]:
...
def get_cached_filtered_documents(filter_params: Dict[str, Any]) -> Optional[List[Dict]]:
    """
...
def clear_cache(cache_type: Optional[str] = None):
    ...
    if cache_type is None or cache_type == "documents":
        keys_to_remove = [k for k in st.session_state.keys() if k.startswith("doc_cache_")]
        for key in keys_to_remove:
            del st.session_state[key]
    
    if cache_type is None or cache_type == "cost_centers":
        keys_to_remove = [k for k in st.session_state.keys() if k.startswith("cost_centers_cache_")]
        for key in keys_to_remove:
            del st.session_state[key]
    ...
    if cache_type is None or cache_type == "filters":
        keys_to_remove = [k for k in st.session_state.keys() if k.startswith("filtered_docs_")]
        for key in keys_to_remove:
            del st.session_state[key]
...
def get_cache_stats() -> Dict[str, Any]:
    ...
    stats["total_cache_entries"] = (
        stats["document_caches"] +
        stats["cost_center_caches"] +
        stats["filter_caches"]
    )
